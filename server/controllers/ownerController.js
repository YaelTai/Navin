...
class OwnerController {
  logIn = async (req, res) => {
    ...
    if (!(req.body.Password === OwnerToCheck.Password))
      return res
        .status(400)
        .json({ message: "password does not match to user name" });
  ...
  updatePassword = async (req, res) => {
    if (!(await OwnerDB.updatePassword(req.body.Id, req.body.Password)))
      return res
        .status(400)
        .json({ message: "error occured while updating password" });
  ...
  getFee = async (req, res) => {
    ...
    res
      .status(201)
      .json(
        priceList.dataValues.DayFee * daydiff +
          priceList.dataValues.CategoryFee * req.body.numOfCategories
      );
  };
  getPriceList = async (req, res) => {
    ...
    if (!priceList)
      res
        .status(400)
        .json({ message: "error occured while getting price list" });
  ...
  getStoreDetails = async (req, res) => {
    ...
    if (!details)
      res
        .status(400)
        .json({ message: "error occured while getting store details" });
  ...
  getPersonalDetails = async (req, res) => {
    ...
    if (!details)
      return res
        .status(400)
        .json({ message: "error occured while getting owner details" });
  ...
  updateStoreDetails = async (req, res) => {
    ...
    let store = await StoreDB.getStoreByName(req.body.Name);
    let storeId = store.dataValues.Id;
    console.log("the store id is: ", store.dataValues.Id);
    if (!storeId)
      return res
        .status(400)
        .json({
          message:
            "error occured while update store details when trying get storeId",
        });
    ...
    if (!oldCategories)
      return res
        ...
        .json({
          message:
            "error occured while update store details  when trying get oldCategories",
        });
    let newCategories = req.body.Categories;
    console.log("start update", oldCategories, newCategories);
    //delete categories
      ...
      if (!newCategories.includes(element)) {
        ...
        if (!categoryId)
          return res
            .status(400)
            .json({ message: "error occured while update store details" });
        if (!(await StoreDB.deleteCategoryFromStore(storeId, categoryId)))
          return res
            .status(400)
            .json({ message: "error occured when trying get categoryId" });
      }
    });
    console.log("deleted");
    //add categories
    await newCategories.forEach(async (element) => {
      if (!oldCategories.includes(element)) {
        ...
        if (!(await StoreDB.addCategoryToStore(storeId, categoryId)))
          return res
            .status(400)
            .json({ message: "error occured while update category for store" });
      }
    });
...
res.status(200).json({ message: "updated sucssfully" });
  ...
  updatePersonalDetails = async (req, res) => {
    if (!(await OwnerDB.updatePersonalDetails(req.body)))
      res
        .status(401)
        .json({ message: "error occured while updating owner details" });
  ...
  uploadAd = async (req, res) => {
    ...
    if (!newAd)
      return res
        .status(400)
        .json({ message: "error occured when trying to add ad" });
  ...
  getAdByPassword = async (req, res) => {
    ...
    if (ad.ApprovmentCode != req.body.code)
    ...
    res
      ...
      .json(
        priceList.dataValues.DayFee * daydiff +
          priceList.dataValues.CategoryFee * catsForAd.length
      );
  };
  ...
  payForAd = async (req, res) => {
    ...
    if (!(await AdvertismentDB.UpdatePaid(req.body.Id)))
      return res
        .status(400)
        .json({ message: "error occured when update paid" });
  ...
  getAllYourStores = async (req, res) => {
    ...
    if (!stores)
      return res
        .status(400)
        .json({ message: "error occured when trying to get stores" });
  ...
  getAllyourAds = async (req, res) => {
    ...
    if (!ads)
      return res
        .status(400)
        .json({ message: "error occured when trying to get ads" });
    console.log(ads);
  ...
  getAllCategoriesForStore = async (req, res) => {
    ...
    if (!Categories)
      return res
        .status(400)
        .json({ message: "error occured when get categories" });
